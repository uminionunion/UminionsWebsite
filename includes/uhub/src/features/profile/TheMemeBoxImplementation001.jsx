...
<div style={styles.voteSection}>
  <div className="meme-vote-item" style={{ ...styles.voteCount, cursor: "pointer" }} onClick={(event) => { event.stopPropagation(); handleUpvote(); }} role="button" tabIndex={0}>
    <div style={{ ...styles.voteNumber, pointerEvents: "none" }}>{EMOJIS.UPVOTE_INITIAL} {displayPost.upvotes}</div>
    <div className="meme-vote-label-fit" style={styles.voteLabel}>Upvotes</div>
  </div>
  <div className="meme-vote-item" style={{ ...styles.voteCount, cursor: "pointer" }} onClick={(event) => { event.stopPropagation(); handleDownvote(); }} role="button" tabIndex={0}> 
    <div style={{ ...styles.voteNumber, pointerEvents: "none" }}>{EMOJIS.DOWNVOTE_INITIAL} {displayPost.downvotes}</div>
    <div className="meme-vote-label-fit" style={styles.voteLabel}>Downvotes</div>
  </div>
</div>
  ...
  {isMultiColumnLayout ? (
    <>
      ...
      >
        ...
        <div className="meme-vote-label-fit" style={{ ...styles.voteLabel, ...(displayPost.isFavorited ? { color: "#00ff00" } : {}) }}>Favorites</div>
      </div>
      <div className="meme-vote-item" style={{ ...styles.voteCount, cursor: "pointer" }} onClick={(event) => { event.stopPropagation(); openViewCommentsDialog(); }} role="button" tabIndex={0}>
        <div style={{ ...styles.voteNumber, pointerEvents: "none" }}> 
          {EMOJIS.COMMENT_INITIAL} {displayPost.comments.length}
        </div>
        <div className="meme-vote-label-fit" style={styles.voteLabel}>Comments</div>
      </div>
      <div className="meme-vote-item" style={{ ...styles.voteCount, cursor: "pointer" }} onClick={(event) => { event.stopPropagation(); handlePageNavigation(); }} role="button" tabIndex={0}>
        <div style={{ ...styles.voteNumber, pointerEvents: "none" }}>
          <img src="/EmojisForUminionWebsite/GreenEmoji007UserPost.png" width="24" style={{ verticalAlign: "middle" }} />
        </div>
        <div className="meme-vote-label-fit" style={styles.voteLabel}>{getPageTitle()}</div>
      </div>
    </>
  ) : (
    <>
