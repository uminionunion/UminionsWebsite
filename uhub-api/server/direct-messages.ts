...
router.get('/:userId', authenticate, async (req, res) => {
  ...
  if (!(await areFriends(req.user.userId, otherUserId))) return res.status(403).json({ error: 'You must be friends to message this user.' });
...
router.post('/:userId', authenticate, async (req, res) => {
  ...
  if (!content && !attachmentUrl) return res.status(400).json({ error: 'Message or attachment is required.' });
