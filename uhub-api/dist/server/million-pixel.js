...
router.get('/api/million-pixel/inventory', requireAuth, async (req, res) => {
    try {
...
router.get('/api/million-pixel/pixels', async (req, res) => {
    try {
...
router.get('/api/million-pixel/pixels/:x/:y', async (req, res) => {
    try {
...
router.post('/api/million-pixel/pixels/:x/:y/change', requireAuth, async (req, res) => {
    try {
        const userId = req.user.userId;
        // Million-Pixel is disabled for everyone except user ID 1 until Stripe/WooCommerce is finished.
        if (userId !== 1)
            return res.status(403).json({ error: 'Million-Pixel is not yet available.' });
        const x = Number(req.params.x);
        const y = Number(req.params.y);
        ...
        const result = await db.transaction().execute(async (trx) => {
            ...
            if (!inventory)
                inventory = await trx.insertInto('MillionPixelUserTickets').values({ user_id: userId, total_pixeltickets: 1000 }).returningAll().executeTakeFirstOrThrow();
            ...
            if (changeCount >= 10)
                throw new Error('This pixel has reached maximum changes.');
            const cost = costs[changeCount];
            if (inventory.total_pixeltickets < cost)
                throw new Error(`Insufficient PixelTickets. Need ${cost}, have ${inventory.total_pixeltickets}.`);
