...
router.post('/', authenticate, async (req, res) => {
    ...
    console.log('[PRODUCTS] req.files:', req.files);
...
router.patch('/:productId', authenticate, async (req, res) => {
    ...
    try {
        ...
        if (req.body.price !== undefined)
            updateData.price = req.body.price ? parseFloat(req.body.price) : null;
        if (req.body.payment_method)
            updateData.payment_method = req.body.payment_method;
        if (req.body.payment_url !== undefined)
            updateData.payment_url = req.body.payment_url || null;
        if (req.body.sku_id !== undefined)
            updateData.sku_id = req.body.sku_id || null;
        if (req.body.store_id !== undefined)
            updateData.store_id = parseInt(req.body.store_id) || null;
...
router.get('/cart', authenticate, async (req, res) => {
    ...
    try {
        const cartItems = await db
            .selectFrom('MainHubUpgradeV001ForInternalCart')
            .leftJoin('MainHubUpgradeV001ForProducts', 'MainHubUpgradeV001ForInternalCart.product_id', 'MainHubUpgradeV001ForProducts.id')
...
router.get('/user/:userId/stores', async (req, res) => {
    ...
    try {
        ...
        const stores = await db
            ...
            .leftJoin('MainHubUpgradeV001ForProducts as p', 'us.id', 'p.user_store_id')
            .selectAll('us')
            .select([
            'p.id as product_id',
...
router.post('/user/:userId/stores', authenticate, async (req, res) => {
    ...
    try {
        ...
        if (req.files && req.files.badgeImage) {
            await ensureUploadDir();
            const uploadedFile = req.files.badgeImage;
        ...
        if (req.files && req.files.bannerImage) {
            await ensureUploadDir();
            const uploadedFile = req.files.bannerImage;
            const uploadsDir = path.join(process.cwd(), 'data', 'uploads', 'store-images');
            await fs.mkdir(uploadsDir, { recursive: true });
            const ext = uploadedFile.name.split('.').pop()?.toLowerCase() || 'jpg';
        ...
        if (user.is_new_user === 1) {
            const storeCount = await db
                ...
                .executeTakeFirst();
            const currentCount = storeCount?.count || 0;
...
router.get('/friends/stores/all', authenticate, async (req, res) => {
    ...
    try {
        ...
        // Extract friend IDs
        const friendIds = friendships.map(f => f.user_id1 === req.user.userId ? f.user_id2 : f.user_id1);
        ...
        friendsUStoresData.forEach(row => {
            ...
            if (!friendsMap.has(row.friend_id)) {
                friendsMap.set(row.friend_id, {
                    ...
                    uStores: new Map(), // uStore ID → uStore data
                });
            }
