...
function warnIfUrlAndStack(s) {
    if (process.env.DEBUG_ROUTE_SANITIZE !== 'true')
        return;
    if (!s)
        return;
...
function patchMethodOn(target, methodName) {
    if (!target || typeof target[methodName] !== 'function')
        return;
    const orig = target[methodName];
    target[methodName] = function (arg1, ...rest) {
        try {
...
app.post('/api/broadcasts/union-news-14/images/add', authMiddleware, async (req, res) => {
    try {
        if (!req.user?.userId) {
            res.status(401).json({ error: 'Not authenticated' });
            return;
...
app.get('/api/broadcasts/union-news-14/images', async (req, res) => {
    try {
...
app.post('/api/broadcasts/union-news-14/images/:id/move-left', authMiddleware, async (req, res) => {
    try {
        if (!req.user?.userId) {
            res.status(401).json({ error: 'Not authenticated' });
            return;
...
app.post('/api/broadcasts/union-news-14/images/:id/move-right', authMiddleware, async (req, res) => {
    try {
        if (!req.user?.userId) {
            res.status(401).json({ error: 'Not authenticated' });
            return;
...
app.get('/api/chat/rooms-with-messages', async (req, res) => {
    try {
...
app.get('/api/chat/unread-status', authMiddleware, async (req, res) => {
    try {
        if (!req.user?.userId) {
            res.status(401).json({ error: 'Not authenticated' });
            return;
...
app.post('/api/chat/mark-as-read', authMiddleware, async (req, res) => {
    try {
        if (!req.user?.userId) {
            res.status(401).json({ error: 'Not authenticated' });
            return;
