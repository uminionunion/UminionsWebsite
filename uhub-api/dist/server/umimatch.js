...
const router = Router();
const asString = (value) => Array.isArray(value) ? String(value[0] || '') : String(value || '');
router.get('/api/umimatch/profile', requireAuth, async (req, res) => {
    try {
...
router.put('/api/umimatch/profile', requireAuth, async (req, res) => {
    try {
        const userId = req.user.userId;
        const updates = {
            ...
            allow_anyone: req.body.allow_anyone ? 1 : 0,
...
router.post('/api/umimatch/skip', requireAuth, async (req, res) => {
    try {
        const userId = req.user.userId;
...
router.post('/api/umimatch/like', requireAuth, async (req, res) => {
    try {
        const userId = req.user.userId;
        const targetUserId = Number(req.body.targetUserId);
        if (!Number.isFinite(targetUserId) || targetUserId === userId)
...
router.get('/api/umimatch/matches', requireAuth, async (req, res) => {
    try {
        const userId = req.user.userId;
...
router.get('/api/umimatch/messages/:userId', requireAuth, async (req, res) => {
    try {
        const userId = req.user.userId;
...
router.post('/api/umimatch/messages/:userId', requireAuth, async (req, res) => {
    try {
